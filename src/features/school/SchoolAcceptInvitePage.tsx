...
import { acceptSchoolInvite, previewSchoolInvite, signUpSchoolUser } from "../../lib/schoolPortal";
import { formatDate } from "../../lib/format";

type Preview = Awaited<ReturnType<typeof previewSchoolInvite>>;

export function SchoolAcceptInvitePage() {
  const [params] = useSearchParams();
  const token = params.get("token") || sessionStorage.getItem("school-portal-invite-token") || "";
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!token) return;
    sessionStorage.setItem("school-portal-invite-token", token);
    previewSchoolInvite(token)
      .then((result) => {
        ...
        else if (result.invite_state === "Used") setError("This invitation has already been used. Log in to your school portal.");
        setPreview(result);
  ...
  const acceptExisting = async () => {
    ...
    setIsSaving(true);
    try {
      await acceptSchoolInvite(token);
      sessionStorage.removeItem("school-portal-invite-token");
  ...
  const createAccount = async (event: FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const session = await signUpSchoolUser(email, password);
      if (session) {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to create school account.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 dark:bg-slate-950 dark:text-white">
      <Card className="mx-auto mt-10 max-w-lg">
        {isLoading ? <Skeleton className="h-44 w-full" /> : (
          <>
            <p className="text-sm font-semibold uppercase text-brand-600 dark:text-brand-100">School invitation</p>
            <h1 className="mt-3 text-2xl font-bold">{preview?.school_name || "School Portal"}</h1>
            <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
              {preview ? `${preview.agency_name} invited you as ${preview.role}. This invite expires ${formatDate(preview.expires_at)}.` : "Your school invitation could not be loaded."}
            </p>
            ...
            {preview && preview.invite_state === "Valid" && user ? (
              <Button className="mt-6 w-full" disabled={isSaving} onClick={acceptExisting}>{isSaving ? "Activating..." : "Accept invitation"}</Button>
              ...
              <form className="mt-6 space-y-4" onSubmit={createAccount}>
                ...
                <Button className="w-full" type="submit" disabled={isSaving}>{isSaving ? "Creating account..." : "Create school account"}</Button>
              </form>
            ) : null}
          </>
        )}
